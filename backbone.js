...
(function(){
  ...
  _.extend(Model.prototype, Events, {
    ...
    set: function(key, value, options) {
      ...
      for (attr in attrs) {
        ...
        // 이전 값이 다른 경우 변경 사항을 기록하고, 그렇지 않은 경우 이 속성의 변경 내용을 삭제한다.
    ...
    // 모델에서 속성을 제거하고, silent를 선택하지 않는 한 `"change"` 이벤트를 실행한다.
    ...
    // 모델의 모든 속성을 지우고 silent를 선택하지 않는 한`  "change"` 이벤트를 실행한다.
  ...
  _.extend(History.prototype, Events, {
    ...
    start: function(options) {
      ...
      // 초기 설정을 찾는다. iframe이 필요한가? pushState가 필요하지만, 그것이 유효한가?
    ...
    // 아마 일시적으로 Backbone.history을 무효로 한다. 실제 응용 프로그램에서 사용할 수 없지만,
    ...
    // 현재 URL fragment의 로드를 시도한다. 매치하여 경로가 성공하면 `true` 을 돌려 준다.
    ...
    // `trigger: true`가 포함되는게 가능하며, 또한 이력에 현재 URL를 추가하지 않고 변경하고자
    ...
    navigate: function(fragment, options) {
      ...
      // 갱신한다.
        ...
        if (this.iframe && (frag != this.getFragment(this.getHash(this.iframe)))) {
          ...
          // IE7 이전에서 해시의 변경을 저장하기 위해 iframe을 열고-닫는 트릭을 시행한다.
